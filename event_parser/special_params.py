...
def get_special_param_str(special_param: str, param: int) -> str:
    default = f"{param} "
    if(special_param in globals()):
        global_obj = globals()[special_param]
        if(param in global_obj):
            return f"{global_obj[param]}({param}) "
        else:
            return default
    else:
        print(f"WARNING: Unknown special param: {special_param}")
        return default
...
class SpecialParamsTracker:
	...
	def add(self, special_param: str, param_value: int):
		# track that it used this special param -- initialize it as a set if this is the first time it's been seen here
		if(special_param not in self.special_params_used):
			self.special_params_used[special_param] = set()
		self.special_params_used[special_param].add(param_value)
